# KlipperPlace OpenPNP API Schema

## Overview

This document defines the complete API endpoint schema for KlipperPlace OpenPNP integration. The API provides REST endpoints for command execution and WebSocket endpoints for real-time updates, enabling seamless integration between OpenPNP 2.0 and Klipper firmware.
...
#### PICK_AND_PLACE - Complete Pick and Place

Execute complete pick and place operation.

**Endpoint:** `POST /pnp/pick_and_place`
...
#### ACTUATE - Control Actuator

Control an actuator (GPIO pin).

**Endpoint:** `POST /actuators/actuate`
...
#### ACTUATOR_ON - Turn On Actuator

Turn on an actuator.

**Endpoint:** `POST /actuators/on`
...
#### ACTUATOR_OFF - Turn Off Actuator

Turn off an actuator.

**Endpoint:** `POST /actuators/off`
...
#### VACUUM_ON - Enable Vacuum

Enable vacuum pump.

**Endpoint:** `POST /vacuum/on`
...
#### VACUUM_OFF - Disable Vacuum

Disable vacuum pump.

**Endpoint:** `POST /vacuum/off`
...
#### VACUUM_SET - Set Vacuum Power

Set vacuum power level.

**Endpoint:** `POST /vacuum/set`
...
#### FAN_SET - Set Fan Speed

Set fan speed.

**Endpoint:** `POST /fan/set`
...
#### PWM_RAMP - Ramp PWM Value

Ramp PWM value over time.

**Endpoint:** `POST /pwm/ramp`
...
#### GPIO_WRITE - Write GPIO Pin

Write GPIO pin state.

**Endpoint:** `POST /gpio/write`
...
#### GPIO_READ_ALL - Read All GPIO Pins

Read all configured GPIO pin states.

**Endpoint:** `GET /gpio/read_all`
...
#### SENSOR_READ_ALL - Read All Sensors

Read all configured sensor values.

**Endpoint:** `GET /sensor/read_all`
...
#### SENSOR_READ_BY_TYPE - Read Sensors by Type

Read all sensors of a specific type.

**Endpoint:** `GET /sensor/read_by_type`
...
#### FEEDER_ADVANCE - Advance Feeder

Advance feeder by specified distance.

**Endpoint:** `POST /feeder/advance`
...
#### FEEDER_RETRACT - Retract Feeder

Retract feeder by specified distance.

**Endpoint:** `POST /feeder/retract`
...
#### GET_PRINTER_STATE - Get Printer State

Get printer state information.

**Endpoint:** `GET /printer/state`
...
#### QUEUE_ADD - Add Command to Queue
...
#### QUEUE_BATCH - Add Multiple Commands to Queue
...
#### QUEUE_CANCEL - Cancel Specific Command

Cancel a specific queued command.

**Endpoint:** `DELETE /queue/cancel`
...
**Endpoint:** `POST /system/emergency_stop`
...
#### BATCH_EXECUTE - Execute Batch Commands
...
sequenceDiagram
    ...
    participant Moonraker as Moonraker

    Client->>API: Request with X-API-Key header
    API->>Config: Load API key
    Config-->>API: API key value
    API->>API: Validate API key
    alt Valid API Key
        API->>Moonraker: Execute command
        Moonraker-->>API: Result
        API-->>Client: 200 OK + Response
    else Invalid API Key
        API-->>Client: 401 Unauthorized
    end
...
|---------|-------------|----------|
| X-API-Key | API authentication key | X-API-Key: abc123def456 |
...
This API can be described using OpenAPI 3.0 specification for automatic client generation.
...
openapi: 3.0.0
info:
  title: KlipperPlace OpenPNP API
  version: 1.0.0
  description: API for KlipperPlace OpenPNP integration
  contact:
    name: KlipperPlace Project
    url: https://github.com/klipperplace
servers:
  - url: http://localhost:7125/api/v1
  ...
  schemas:
    ...
    SuccessResponse:
      ...
      properties:
        status:
          type: string
          enum: [success, error, partial, timeout, cancelled]
        command:
          type: string
        command_id:
          type: string
